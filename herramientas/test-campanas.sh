#!/bin/zsh
# Comprobaciones de las campañas de Mailing. Monta el mismo bundle que la prueba
# de humo (los <script> de index.html) y le añade los asserts de campanas.
set -e
cd "$(git rev-parse --show-toplevel)"

DIR=$(mktemp -d "${TMPDIR:-/tmp}/malo-camp.XXXXXX")
...
else echo "  !!  sin node ni jsc: me salto las comprobaciones"; exit 0; fi
...
cat "$DIR/app.js" >> "$RUN"
cat herramientas/test-campanas.js >> "$RUN"
